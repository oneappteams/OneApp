...
public class SixthTabFragment extends Fragment {
    ...
    public static SixthTabFragment newInstance(String contentUrl, Activity screen, String homePageUrl, String tabName) {
        SixthTabFragment fragment = new SixthTabFragment();
        activity = screen;
    ...
    public void startHomePage(){
        MainActivity.homeClickedSixthTab = false;
        if(null != activity) {
            SharedPreferences sharedpreferences = activity.getSharedPreferences(
                    MainActivity.My_preference, Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedpreferences.edit();
        ...
        if (!isVisibleToUser) {
            ...
            if (activity != null && Utils.isNetworkAvailable(activity)) {
                ...
                if (isVisibleToUser && !_areLecturesLoaded && activityLoaded) {
                    ...
                    String urlTotal = sharedpreferences.getString(tabTitle.toLowerCase(), "");
                    List<String> urlList = Arrays.asList(urlTotal.split("_tokenizer_url:::search_app_"));
                    if (MainActivity.homeClickedSixthTab) {
                        startHomePage();
                    } else {
                        if (MainActivity.searchClickedSixthTab) {
                            MainActivity.searchClickedSixthTab = false;
                            webViewPresent.setVisibility(View.VISIBLE);
                            webViewPast.setVisibility(View.GONE);
